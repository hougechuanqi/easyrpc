...
public class ReflectionUtils {
	...
	private static void setJobConf(Object theObject, Configuration conf) {
		...
		try {
			Class<?> jobConfClass = conf
					.getClassByNameOrNull("org.apache.hadoop.mapred.JobConf");
			if (jobConfClass == null) {
				return;
			}

			Class<?> jobConfigurableClass = conf
					.getClassByNameOrNull("org.apache.hadoop.mapred.JobConfigurable");
			...
			if (jobConfClass.isAssignableFrom(conf.getClass())
					&& jobConfigurableClass.isAssignableFrom(theObject
							.getClass())) {
				Method configureMethod = jobConfigurableClass.getMethod(
						"configure", jobConfClass);
		...
		if (log.isInfoEnabled()) {
			...
			if (dumpStack) {
				try {
					...
					printThreadInfo(new PrintStream(buffer, false, "UTF-8"),
							title);
