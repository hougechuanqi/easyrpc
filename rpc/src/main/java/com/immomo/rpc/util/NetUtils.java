...
import com.immomo.rpc.io.SocketIOWithTimeout;
import com.immomo.rpc.io.SocketInputStream;
import com.immomo.rpc.io.SocketInputWrapper;
import com.immomo.rpc.io.SocketOutputStream;
...
public class NetUtils {
	...
	private static String canonicalizeHost(String host) {
		...
		if (fqHost == null) {
			try {
				fqHost = InetAddress.getByName(host).getHostName();
	 ...
	 * "0.0.0.0". This returns "hostname:port" of the server, or
	 * "127.0.0.1:port" when the getListenerAddress() returns "0.0.0.0:port".
	 * 
	 * @param server
	...
	/**
	 * Same as <code>getInputStream(socket, socket.getSoTimeout()).</code> <br>
	 * <br>
	 * 
	 ...
	 * socket timeout will be set to the specified value. Otherwise, a
	 ...
	 * @param socket
	 * @param timeout
	 *            timeout in milliseconds. zero for waiting as long as
	 *            necessary.
	...
	public static SocketInputWrapper getInputStream(Socket socket, long timeout)
			throws IOException {
		InputStream stm = (socket.getChannel() == null) ? socket
				.getInputStream() : new SocketInputStream(socket);
	...
	/**
	 * Like {@link NetUtils#connect(Socket, SocketAddress, int)} but also takes
	 * a local address and port to bind the socket to.
	 * 
	 * @param socket
	 * @param endpoint
	 *            the remote address
	...
	public static IOException wrapException(final String destHost,
		...
		if (exception instanceof BindException) {
			return wrapWithMessage(exception, "Problem binding to ["
					+ localHost + ":" + localPort + "] " + exception + ";"
					+ see("BindException"));
			...
			return wrapWithMessage(exception, "End of File Exception between "
					+ getHostDetailsAsString(destHost, destPort, localHost)
					+ ": " + exception + ";" + see("EOFException"));
		} else {
