...
public class ArrayPrimitiveWritable implements Writable {
	...
	// ArrayPrimitiveWritable(Class<?>) constructor), it will provide
	// typechecking
	...
	private static void checkPrimitive(Class<?> componentType) {
		...
		if (!PRIMITIVE_NAMES.containsKey(componentType.getName())) {
			throw new IllegalArgumentException("input array component type "
					...
					+ " is not a candidate primitive type");
		}
	}
	...
	/**
	 * Construct an instance of known type but no value yet for use with
	...
	/**
	 * Get the original array. Client must cast it back to type componentType[]
	...
	/**
	 * Do not use this class. This is an internal class, purely for
	 * ObjectWritable to use as a label class for transparent conversions of
	 * arrays of primitives during wire protocol reads and writes.
	 */
	...
	public void write(DataOutput out) throws IOException {
		// write componentType
		MsgPackUtils.writeString(out, componentType.getName());
		// write length
	...
	public void readFields(DataInput in) throws IOException {
		...
		if (componentType == null) {
			throw new IOException("encoded array component type " + className
					+ " is not a candidate primitive type");
		}
		checkDeclaredComponentType(componentType);
		this.componentType = componentType;
		...
		if (componentType == Boolean.TYPE) { // boolean
			...
			throw new IOException("Encoded type " + className
					+ " converted to valid component type "
					+ componentType.toString()
