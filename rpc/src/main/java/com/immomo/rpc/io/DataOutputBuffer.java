...
public class DataOutputBuffer extends DataOutputStream {
	...
	private static class Buffer extends ByteArrayOutputStream {
		...
		private int setCount(int newCount) {
			Preconditions
					.checkArgument(newCount >= 0 && newCount <= buf.length);
			int oldCount = count;
			count = newCount;
			return oldCount;
		}
	...
	/**
	 * Overwrite an integer into the internal buffer. Note that this call can
	 * only be used to overwrite existing data in the buffer, i.e., buffer#count
	 * cannot be increased, and DataOutputStream#written cannot be increased.
	 */
